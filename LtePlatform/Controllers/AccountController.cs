        ...
        public async Task<JsonResult> LoginExternal(LoginExternalViewModel model)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    var roles =
                        await (await _httpClient.GetAsync("/api/UserRoles?userName=" + model.UserName)).Content.ReadAsAsync(
                            Type.GetType("System.String"));
                    await _httpClient.GetAsync("/api/UserRecord?userName=" + model.UserName + "&token=" + token
                        + "&roles=" + roles);
                    return Json(new
                    {
                        Result = "Success",
