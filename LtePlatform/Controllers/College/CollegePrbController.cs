...
{
    [ApiControl("校园网PRB利用率查询控制器")]
    [ApiGroup("专题优化")]
    public class CollegePrbController : ApiController
    {
        ...
        public CollegePrbController(PrbQueryService service, CollegeCellViewService collegeCellViewService,
            CollegeStatService collegeService, TownPrbService townPrbService)
        {
            _service = service;
        ...
        public IEnumerable<AggregatePrbView> Get(DateTime startDate, DateTime lastDate)
        {
            ...
            {
                var stats = _townPrbService.QueryTownPrbViews(startDate, lastDate, college.Id, FrequencyBandType.College);
        ...
        [HttpGet]
        [ApiDoc("查询所有学校指定日期范围内PRB利用率情况，按照日期排列")]
        ...
        public IEnumerable<AggregatePrbView> GetAllDateViews(DateTime firstDate, DateTime secondDate)
        {
            ...
            {
                var stat = _townPrbService.QueryOneDateBandStat(begin, FrequencyBandType.College);
        ...
        [HttpGet]
        [ApiDoc("查询指定日期内所有学校PRB利用率情况")]
        [ApiParameterDoc("currentDate", "指定日期")]
        [ApiResponse("所有学校天PRB利用率统计")]
        public IEnumerable<AggregatePrbView> Get(DateTime currentDate)
        {
            ...
            return colleges.Select(college =>
            {
                var stats = _townPrbService.QueryTownPrbViews(currentDate, lastDate, college.Id, FrequencyBandType.College);
                var result = stats.Any()
                    ? stats.ArraySum().MapTo<AggregatePrbView>()
                    : new AggregatePrbView();
                result.Name = college.Name;
        ...
        [HttpGet]
        [ApiDoc("查询指定学校指定日期范围内PRB利用率情况")]
        [ApiParameterDoc("collegeName", "学校名称")]
        [ApiParameterDoc("begin", "开始日期")]
        [ApiParameterDoc("end", "结束日期")]
        [ApiResponse("天平均PRB利用率统计")]
        public AggregatePrbView Get(string collegeName, DateTime begin, DateTime end)
        {
            var college = _collegeService.QueryInfo(collegeName);
            if (college == null) return null;
            var stats = _townPrbService.QueryTownPrbViews(begin, end, college.Id, FrequencyBandType.College);
            var result = stats.Any()
                ? stats.ArraySum().MapTo<AggregatePrbView>()
                : new AggregatePrbView();
        ...
        [HttpGet]
        [ApiDoc("查询指定学校指定日期范围内PRB利用率情况，按照日期排列")]
        [ApiParameterDoc("collegeName", "学校名称")]
        [ApiParameterDoc("beginDate", "开始日期")]
        [ApiParameterDoc("endDate", "结束日期")]
        [ApiResponse("PRB利用率情况，按照日期排列，每天一条记录")]
        public IEnumerable<AggregatePrbView> GetDateViews(string collegeName, DateTime beginDate, DateTime endDate)
        {
            var college = _collegeService.QueryInfo(collegeName);
            if (college == null) return null;
            var stats = _townPrbService.QueryTownPrbViews(beginDate, endDate, college.Id, FrequencyBandType.College);
        ...
        [HttpGet]
        [ApiDoc("查询指定学校指定日期各个小区PRB利用率情况")]
        [ApiParameterDoc("collegeName", "学校名称")]
        [ApiParameterDoc("statDate", "统计日期")]
        [ApiResponse("各个小区PRB利用率情况统计")]
        public IEnumerable<PrbView> GetCollegeDatePrbView(string collegeName, DateTime statDate)
        {
        ...
        [HttpGet]
        [ApiDoc("抽取查询单日所有校园网的PRB利用率统计（导入采用，一般前端代码不要用这个接口）")]
        [ApiParameterDoc("statDate", "统计日期")]
        [ApiResponse("所有校园网的PRB利用率统计")]
