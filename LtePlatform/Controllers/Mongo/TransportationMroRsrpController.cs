...
{
    [ApiControl("交通枢纽MRO覆盖查询控制器")]
    [ApiGroup("专题优化")]
    public class TransportationMroRsrpController : ApiController
    {
        ...
        public TransportationMroRsrpController(TopCoverageStatService service, CollegeCellViewService collegeCellViewService,
        ...
        [HttpGet]
        [ApiDoc("查询所有交通枢纽指定日期范围内MRO覆盖情况，按照日期排列")]
        ...
        public IEnumerable<AggregateCoverageView> GetAllDateViews(DateTime firstDate, DateTime secondDate)
        {
            ...
            {
                var stat = _townMroRsrpService.QueryOneDateBandStat(begin, FrequencyBandType.Transportation);
        ...
        [HttpGet]
        [ApiDoc("查询指定日期内所有交通枢纽MRO覆盖情况")]
        [ApiParameterDoc("currentDate", "指定日期")]
        [ApiResponse("所有交通枢纽天MRO覆盖统计")]
        public IEnumerable<AggregateCoverageView> Get(DateTime currentDate)
        {
            ...
            return colleges.Select(college =>
            {
                var stats = _townMroRsrpService.QueryTownViews(currentDate, lastDate, college.Id, FrequencyBandType.Transportation);
                var result = stats.Any()
                    ? stats.ArraySum().MapTo<AggregateCoverageView>()
                    : new AggregateCoverageView();
                result.Name = college.HotspotName;
        ...
        [HttpGet]
        [ApiDoc("查询指定交通枢纽指定日期范围内MRO覆盖情况")]
        [ApiParameterDoc("transportationName", "交通枢纽名称")]
        [ApiParameterDoc("begin", "开始日期")]
        [ApiParameterDoc("end", "结束日期")]
        [ApiResponse("天平均MRO覆盖统计")]
        public AggregateCoverageView Get(string transportationName, DateTime begin, DateTime end)
        {
            var college = _marketService.QueryTransportationView(transportationName);
            if (college == null) return null;
            var stats = _townMroRsrpService.QueryTownViews(begin, end, college.Id, FrequencyBandType.Transportation);
            var result = stats.Any()
                ? stats.ArraySum().MapTo<AggregateCoverageView>()
                : new AggregateCoverageView();
        ...
        [HttpGet]
        [ApiDoc("查询指定交通枢纽指定日期范围内MRO覆盖情况，按照日期排列")]
        [ApiParameterDoc("transportationName", "交通枢纽名称")]
        [ApiParameterDoc("beginDate", "开始日期")]
        [ApiParameterDoc("endDate", "结束日期")]
        [ApiResponse("MRO覆盖情况，按照日期排列，每天一条记录")]
        public IEnumerable<AggregateCoverageView> GetDateViews(string transportationName, DateTime beginDate, DateTime endDate)
        {
            var college = _marketService.QueryTransportationView(transportationName);
            if (college == null) return null;
            var stats = _townMroRsrpService.QueryTownViews(beginDate, endDate, college.Id, FrequencyBandType.Transportation);
        ...
        [HttpGet]
        [ApiDoc("查询指定交通枢纽指定日期各个小区MRO覆盖情况")]
        [ApiParameterDoc("transportationName", "交通枢纽名称")]
        [ApiParameterDoc("statDate", "统计日期")]
        [ApiResponse("各个小区MRO覆盖情况统计")]
        public IEnumerable<CoverageStatView> GetTransportationDateView(string transportationName, DateTime statDate)
        {
