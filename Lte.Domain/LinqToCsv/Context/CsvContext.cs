...
namespace Lte.Domain.LinqToCsv.Context
{
    ...
    public static class CsvContext
    {
        ...
        public static IEnumerable<T> Read<T>(StreamReader stream, CsvFileDescription fileDescription) 
            where T : class, new()
        {
            var dataAccess = new FileDataAccess(stream, fileDescription);
        ...
        {
            using (var sw = new StreamWriter(fileName, false, fileDescription.TextEncoding))
            {
            ...
            CsvFileDescription fileDescription)
        {
            ...
            var cs = new CsvStream(null, stream, fileDescription.SeparatorChar, 
