...
namespace Lte.Domain.LinqToExcel.Entities
{
    public class WhereClauseExpressionTreeVisitor : ThrowingExpressionTreeVisitor
    {
        ...
        public IEnumerable<OleDbParameter> Params => _params;
        ...
        protected override Expression VisitBinaryExpression(BinaryExpression bExp)
        {
            ...
            switch (bExp.NodeType)
            {
                ...
                case ExpressionType.Equal:
                    _whereClause.Append(bRight.IsNullValue() ? " IS NULL" : " = ");
                    break;
                ...
                case ExpressionType.NotEqual:
                    _whereClause.Append(bRight.IsNullValue() ? " IS NOT NULL" : " <> ");
                    break;
        ...
        protected BinaryExpression ConvertVbStringCompare(BinaryExpression exp)
        {
            if (exp.Left.NodeType == ExpressionType.Call)
            {
                ...
                if (compareStringCall.Method.DeclaringType != null 
                    && (compareStringCall.Method.DeclaringType.FullName 
        ...
        /// <summary>
        /// Only As<T/>() method calls on the LinqToExcel.Row type are support
        /// </summary>
        ...
        private void AddStringIsNullOrEmptyToWhereClause(MethodCallExpression mExp, bool notEqual = false)
        {
            ...
            _whereClause.AppendFormat(
                notEqual ? "(({0} <> '') OR ({0} IS NOT NULL))" : "(({0} = '') OR ({0} IS NULL))", columnName);
        }
        ...
        private string GetColumnName(Expression exp)
        {
            ...
            return expression != null ? GetColumnName(expression) : GetColumnName((MethodCallExpression)exp);
        }
        ...
        private string GetColumnName(MethodCallExpression mExp)
        {
            ...
            MethodCallExpression expression = mExp.Object as MethodCallExpression;
            if (expression != null)
                method = expression;
            ...
            {
                if (_sheetType == typeof(ExcelRowNoHeader))
